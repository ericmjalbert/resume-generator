...
from resume_generator.general.model import Basics, Education, Skills, Work
...
class Resume:
    ...
    def update_work(self, new_work):
        self.resume_json["work"] = new_work
        self.work = self.parse_resume_for_field(model=Work, field="work")

    def update_skills(self, new_skills):
        self.resume_json["skills"] = new_skills
        self.skills = self.parse_resume_for_field(model=Skills, field="skills")

    def __str__(self):
        """Printed in JSON format similar to master_resume"""
        return json.dumps(self.resume_json)
